	...
	<table class="woocommerce-cart-form__contents cart-table" cellspacing="0">
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<tr class="woocommerce-cart-form__cart-item cart-table__item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<td class="cart-table__thumbnail product-thumbnail">
						<?php
					...
					<td class="cart-table__name product-name" data-title="<?php esc_attr_e( 'Product', 'woocommerce' ); ?>">
					...
					<td class="cart-table__remove product-remove">
						<?php
							echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								...
								sprintf(
									'<a href="%s" class="product-remove__button remove" aria-label="%s" data-product_id="%s" data-product_sku="%s"><span class="product-remove__desktop-text">&times;</span><span class="product-remove__mobile-text">%s</span></a>',
					...
					<div class="coupon">
						<div class="coupon__field">
							<div class="text-field">
								<input type="text" name="coupon_code" class="text-field__input" id="coupon_code" value="" placeholder="<?php echo __( 'Enter coupon code', 'fitbody-theme' ); ?>" />
								<label for="coupon_code" class="text-field__placeholder"><?php echo __( 'Enter coupon code', 'fitbody-theme' ); ?></label>
							</div>
						</div>
		...
		<tr>
			<td colspan="6" style="text-align: right;">
				<button type="submit" class="button button--bordered" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'woocommerce' ); ?>"><?php esc_html_e( 'Update cart', 'woocommerce' ); ?></button>
